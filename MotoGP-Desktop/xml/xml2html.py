...
html = Html("Información del circuito Ricardo Tormo")
...
if referencias:
    ...
    for ref in referencias:
        ...
        items.append(f'<a href="{enlace}">{texto}</a>')
    html.addList(items)
...
if fotos:
    ...
    items = []
    for foto in fotos:
        archivo = foto.attrib.get('archivo', '').strip()
        if archivo:
            items.append(archivo)
    html.addList(items)
...
if videos:
    ...
    items = []
    for video in videos:
        archivo = video.attrib.get('archivo', '').strip()
        if archivo:
            items.append(archivo)
    html.addList(items)
